...
export async function POST(req: Request) {
  ...
  const cliquesShopeeLive = numeroNoIntervalo(json.valor.cliquesShopeeLive, 0, 100_000_000);
  ...
  const temDetalhamento = cliquesRedesSociais !== null || cliquesShopeeVideo !== null || cliquesShopeeLive !== null;
  ...
  const shopeeLive = Math.floor(cliquesShopeeLive ?? 0);
  const total = temDetalhamento ? redesSociais + shopeeVideo + shopeeLive : Math.floor(cliques || 0);
  ...
  salvarMetricasShopeeDia({
    ...
    cliquesShopeeLive: shopeeLive,
    fonte: "painel_shopee"
  });
  return NextResponse.json({ ok: true, metricas: { cliquesTotal: total, cliquesRedesSociais: redesSociais, cliquesShopeeVideo: shopeeVideo, cliquesShopeeLive: shopeeLive } });
}
