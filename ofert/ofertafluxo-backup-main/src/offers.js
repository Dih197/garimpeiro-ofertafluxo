...
export function selectOffers(offers, filters, sentIds) {
  ...
  // No painel, 0 no teto de preço significa “sem teto”. Sem esse tratamento,
  // uma configuração recém-salva escondia todas as ofertas com preço positivo.
  const configuredMaxPrice = Number(filters.maxPrice || 0);
  const maxPrice = configuredMaxPrice > 0 ? configuredMaxPrice : Number.POSITIVE_INFINITY;
  ...
  return offers
    ...
    .filter(offer => offer.price >= minPrice && offer.price <= maxPrice)
