...
// prioridade se for adicionado posteriormente pelo administrador.
loadEnv(path.resolve('..', '..', 'garimpeiro', '.env.local'));
loadEnv('.env', true);
const base = config();
...
function securityHeaders() {
  return {
    ...
    // Permite que o painel nativo seja usado somente dentro do Garimpeiro local.
    'content-security-policy': "default-src 'self'; script-src 'self'; connect-src 'self'; img-src 'self' https: data:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src https://fonts.gstatic.com; base-uri 'none'; frame-ancestors 'self' http://127.0.0.1:3000 http://localhost:3000; form-action 'self'"
  };
}
...
http.createServer(async (request, response) => {
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/whatsapp-direct/groups/refresh') return json(response, 200, await refreshDirectWhatsAppGroups(user.id));
    ...
    if (request.method === 'POST' && url.pathname === '/api/automation') {
      ...
      if (Boolean(input.enabled)) {
        const settings = appSettings(user);
        const authorizedDestinations = saved.destinations.filter(destination => destination.active && destination.consent);
        if (!settings.shopee.appId || !settings.shopee.secret) throw new Error('Conecte a Shopee antes de ativar a automação.');
        if (settings.directWhatsApp.enabled && directWhatsAppState(user.id).status !== 'conectado') throw new Error('Conecte o WhatsApp direto pelo QR Code antes de ativar a automação.');
        if (!authorizedDestinations.length) throw new Error('Adicione ao menos um grupo autoral ativo e confirme o consentimento antes de ativar a automação.');
      }
    ...
    if (request.method === 'POST' && url.pathname === '/api/automation/preview') {
      const settings = appSettings(user);
      ...
      return json(response, 200, { destination: { name: destination.name, category: category.label }, offer: { title: offer.title, price: offer.price, discount: offer.discount, commissionRate: offer.commissionRate, image: offer.image, url: offer.url } });
    }
