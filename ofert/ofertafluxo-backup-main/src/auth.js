...
export function beginLogin({ username, password }) {
  const name = cleanUsername(username); const user = users().find(item => item.username === name);
  if (!user || !verifyPassword(password, user.passwordHash)) throw new Error('Usuário ou senha inválidos.');
